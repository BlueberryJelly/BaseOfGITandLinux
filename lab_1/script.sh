...
if [ -f data_analysis/research_themes.csv ]; then
    rm -f data_analysis/research_themes.txt
fi
cut -d',' -f3 < faculty_5.csv \
| tr -s '+' '\n' \
| sed 's/^[[:space:]]*//; s/[[:space:]]*$//' \
| sort | uniq | sort \
| awk '{ print $0; count++ } END { print count+0 }' >> data_analysis/research_themes.txt
awk '{ print $0 }' < data_analysis/research_themes.txt
